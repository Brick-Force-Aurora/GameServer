...
public final class ClientboundMyDownloadMapPacket implements IClientboundPacket {
    ...
    public ClientboundMyDownloadMapPacket page(final int page) {
        this.page = page;
        return this;
    }
    ...
    public ClientboundMyDownloadMapPacket count(final int count) {
        this.count = count;
        return this;
    }
    ...
    public ClientboundMyDownloadMapPacket offset(final int offset) {
        this.offset = offset;
        return this;
    }
    ...
    public ClientboundMyDownloadMapPacket regMaps(final RegMap[] regMaps) {
    ...
    public void write(final ByteBuf buffer) {
        ...
        for (int i = this.offset; i < this.offset + this.count; i++) {
            ...
            if (regMap.getDeveloper().isEmpty()) {
                ...
                final byte[] bytes = regMap.getDeveloper().getBytes(StandardCharsets.UTF_16LE);
            ...
            final byte roomType = (byte) (Room.clanMatch | Room.official);
            if (roomType > 255L || roomType < 0L) {
                throw new IllegalArgumentException("Value " + roomType + " is out of bounds of allowed number range of 0 - 255");
            }
            buffer.writeByte(roomType & 0xFF);
            if (regMap.tagMask > 255L || regMap.tagMask < 0L) {
                throw new IllegalArgumentException("Value " + regMap.tagMask + " is out of bounds of allowed number range of 0 - 255");
            }
