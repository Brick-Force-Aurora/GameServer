...
public final class ClientboundChannelPacket implements IClientboundPacket {
    ...
    public ClientboundChannelPacket channels(final Channel[] channels) {
    ...
    public void write(final ByteBuf buffer) {
        ...
        for (final Channel channel : channels) {
            ...
            if (channel.name().isEmpty()) {
                ...
                final byte[] bytes = channel.name().getBytes(StandardCharsets.UTF_16LE);
            ...
            if ("".isEmpty()) {
                ...
                final byte[] bytes = "".getBytes(StandardCharsets.UTF_16LE);
                buffer.writeIntLE(bytes.length);
                buffer.writeBytes(bytes);
            }
