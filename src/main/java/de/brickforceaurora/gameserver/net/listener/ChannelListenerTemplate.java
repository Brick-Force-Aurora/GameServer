...
public class ChannelListenerTemplate implements INetListener {
    ...
    public ChannelListenerTemplate(ChannelManager channelManager){
        this.channelManager = channelManager;
    }

    @PacketHandler
    public void onChannelPlayerList(NetContext<ServerboundChannelPlayerListPacket> context){
        BFClient client = context.client();

        //send player list of current channel where player is in
        client.send(new ClientboundSvcEnterListPacket().clients(channelManager.channels().toArray(Channel[]::new)[0].clients().toArray(BFClient[]::new)));
    }
}
