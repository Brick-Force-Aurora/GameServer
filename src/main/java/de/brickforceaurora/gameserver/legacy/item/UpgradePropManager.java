...
public class UpgradePropManager {
    ...
    public boolean useProp(final int category, final int propIndex) {
        if (!loaded || upgradeCatTable == null || propIndex < 0 || propIndex >= upgradeCatTable.length) {
            return false;
        }
    ...
    private boolean load() {
        try {
            final IJson<?> root = readJson("jar://upgradeprops.json");
    ...
    private void parse(final JsonArray array) {
        ...
        for (final IJson<?> elem : array) {
            ...
            // name
            final String name = o.getAsString("name").trim();

            // props
            final JsonArray propsArray = o.getAsArray("props");
            if (propsArray == null || propsArray.size() != PROP_COUNT) {
                throw new IllegalStateException("Invalid props array for " + name + " (expected " + PROP_COUNT + ")");
            }
            ...
            final UpgradeCategoryPropTable table = new UpgradeCategoryPropTable(name, props);
