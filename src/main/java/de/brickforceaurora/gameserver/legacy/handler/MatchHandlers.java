...
public class MatchHandlers {
    ...
    private static void weaponHeldRatio(final GameServerLogic logic, final MsgReference msgRef) {
        final MatchData matchData = msgRef.matchData;

        final int count = msgRef.msg.msg().readInt();
        for (int i = 0; i < count; i++) {
            final long key = msgRef.msg.msg().readLong();
            final float value = msgRef.msg.msg().readFloat();
        }
    ...
    public static void SendPostLoadInit(final GameServerLogic logic, final ClientReference client) {
    ...
    public static void SendDestroyedBrick(final GameServerLogic logic, final ClientReference client, final int brick,
        final MatchData matchData) {
        SendDestroyedBrick(logic, client, brick, matchData, SendType.UNICAST);
    }

    public static void SendDestroyedBrick(final GameServerLogic logic, final ClientReference client, final int brick,
        final MatchData matchData, final SendType sendType) {
        final MsgBody body = new MsgBody();
    ...
    public static void SendCannons(final GameServerLogic logic, final ClientReference client) {
        final MatchData matchData = client.matchData;

        for (final Map.Entry<Integer, Integer> entry : matchData.usedCannons.entrySet()) {
            SendGetCannon(logic, entry.getValue(), entry.getKey(), matchData, client, SendType.UNICAST);
        }
    }

    public static void SendTrains(final GameServerLogic logic, final ClientReference client) {
        final MatchData matchData = client.matchData;

        for (final Map.Entry<Integer, Integer> entry : matchData.usedTrains.entrySet()) {
            SendGetTrain(logic, entry.getValue(), entry.getKey(), matchData, client, SendType.UNICAST);
        }
    }

    public static void SendGetCannon(final GameServerLogic logic, final int seq, final int brickSeq, final MatchData matchData,
        final ClientReference client, final SendType sendType) {
        final MsgBody body = new MsgBody();
    ...
    public static void SendGetTrain(final GameServerLogic logic, final int seq, final int trainId, final MatchData matchData,
        final ClientReference client, final SendType sendType) {
        final MsgBody body = new MsgBody();
    ...
    private static void loadComplete(final GameServerLogic logic, final MsgReference msgRef) {
        final int crc = msgRef.msg.msg().readInt();
    ...
    private static void matchCountdown(final GameServerLogic logic, final MsgReference msgRef) {
        final MatchData matchData = msgRef.matchData;

        final int countdownTime = msgRef.msg.msg().readInt();
    ...
    public static void SendMatchCountdown(final GameServerLogic logic, final MatchData matchData) {
        final MsgBody body = new MsgBody();
    ...
    private static void timer(final GameServerLogic logic, final MsgReference msgRef) {
        final MatchData matchData = msgRef.matchData;

        final int remainTime = msgRef.msg.msg().readInt();
        final int playTime = msgRef.msg.msg().readInt();
    ...
    public static void SendTimer(final GameServerLogic logic, final ClientReference client) {
        final MatchData matchData = client.matchData;
    ...
    private static void inflictedDamage(final GameServerLogic logic, final MsgReference msgRef) {
        ...
        for (int i = 0; i < count; i++) {
            final int targetSeq = msgRef.msg.msg().readInt();
            ...
            for (final ClientReference c : matchData.clientList) {
                if (c.seq == targetSeq) {
                    target = c;
                    break;
                }
        ...
        final int reward = Math.max(1, totalDamage / 7);
    ...
    public static void respawnTicket(final GameServerLogic logic, final MsgReference msgRef) {
    ...
    private static void killLog(final GameServerLogic logic, final MsgReference msgRef) {
        ...
        final int id = msgRef.msg.msg().readInt();

        final boolean exists = matchData.killLog.stream().anyMatch(k -> k.id() == id);
        if (exists || (id == matchData.lastKillLogId)) {
            return;
        }
        ...
        final byte victimType = msgRef.msg.msg().readByte();
        final int victim = msgRef.msg.msg().readInt();
        final int weaponBy = msgRef.msg.msg().readInt();
        final int slot = msgRef.msg.msg().readInt();
        final int category = msgRef.msg.msg().readInt();
        final int hitpart = msgRef.msg.msg().readInt();
        final int damageLogCount = msgRef.msg.msg().readInt();

        final Map<Integer, Integer> damageLog = new HashMap<>();
        for (int i = 0; i < damageLogCount; i++) {
            final int key = msgRef.msg.msg().readInt();
            final int value = msgRef.msg.msg().readInt();
        ...
        final ClientReference victimClient = matchData.clientList.stream().filter(c -> c.seq == victim).findFirst().orElse(null);
        ...
        } else if (!matchData.deadRedPlayers.contains(victimClient.seq)) {
            matchData.deadRedPlayers.add(victimClient.seq);
        }
        SendDeathCount(logic, victimClient);

        if (killer == victim && !damageLog.isEmpty()) {
            killer = damageLog.entrySet().stream().max(Map.Entry.comparingByValue()).map(Map.Entry::getKey).orElse(killer);
        }

        final int finalKiller = killer;
        final ClientReference killerClient = matchData.clientList.stream().filter(c -> c.seq == finalKiller).findFirst().orElse(null);
        ...
        for (final Map.Entry<Integer, Integer> e : damageLog.entrySet()) {
            final int seq = e.getKey();
            final int dmg = e.getValue();

            if (seq == victim) {
                continue;
            }

            final ClientReference assist = matchData.clientList.stream().filter(c -> c.seq == seq).findFirst().orElse(null);
        ...
        final KillLogEntry killLogEntry = new KillLogEntry(id, killerType, killer, victimType, victim, WeaponBy.fromValue(weaponBy), slot,
            category, hitpart, damageLog);
        ...
        if (killer != victim) {
            ...
            case EXPLOSION:
                ...
                final int totalRed = (int) matchData.redSlots.stream().filter(s -> s.isUsed).count();
                final int totalBlue = (int) matchData.blueSlots.stream().filter(s -> s.isUsed).count();
    ...
    public static void SendDeathCount(final GameServerLogic logic, final ClientReference client) {
        final MatchData matchData = client.matchData;
    ...
    public static void SendKillCount(final GameServerLogic logic, final ClientReference client) {
        final MatchData matchData = client.matchData;
    ...
    public static void SendRoundScore(final GameServerLogic logic, final ClientReference client) {
        final MatchData matchData = client.matchData;
    ...
    public static void SendKillLogEntry(final GameServerLogic logic, final KillLogEntry entry, final MatchData matchData) {
        final MsgBody body = new MsgBody();
    ...
    public static void SendAssistCount(final GameServerLogic logic, final ClientReference client) {
        final MatchData matchData = client.matchData;
    ...
    private static void resultDone(final GameServerLogic logic, final MsgReference msgRef) {
    ...
    private static void breakInto(final GameServerLogic logic, final MsgReference msgRef) {
    ...
    public static void SendBreakInto(final GameServerLogic logic, final ClientReference client, final int reply) {
        final MsgBody body = new MsgBody();
